/* C3_386 Dec-1991 NT -> modern 32-bit Win32 compatibility DLL.
 *
 ...
 * It deliberately has no CRT dependency of its own.
 */
...
/* 1991 CRT allocates only 0x3c bytes around this call and treats STARTUPINFO
 * as its old 0x38-byte prefix. Copy only that prefix from today's 0x44-byte x86 structure. */
...
/* DbgPrint is diagnostic-only here. A no-op avoids trying to forward unknown varargs. */
u32 CDECL compat_DbgPrint(u32 fmt){(void)fmt;return 0;}

/* C3's __global_unwind pushes exactly three arguments and then does add esp,0x0c. */
